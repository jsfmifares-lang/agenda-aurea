  ...
  head: () => ({
    meta: [
      ...
      { title: "Agenda Aurea — Agendamento para cabeleireiro" },
      {
        name: "description",
        content:
          "Agende horários no salão em segundos e receba a confirmação pelo WhatsApp.",
      },
      { name: "theme-color", content: "#221c14" },
      { name: "apple-mobile-web-app-capable", content: "yes" },
      { name: "apple-mobile-web-app-title", content: "Agenda Aurea" },
  ...
  useEffect(() => {
    const { data } = supabase.auth.onAuthStateChange((event) => {
      if (event === "PASSWORD_RECOVERY") {
        void router.navigate({ to: "/auth", search: { modo: "recuperar" }, replace: true });
        return;
      }
